...
import { Condition, Operator, ApiCollectionSchema, ApiCollection, Collection, CollectionType } from "@/types/collection";
            ...
            <Card className="p-4">
              <SectionTitle title="Collection image" />
              {
                collection.image ? (
                  ...
                  <ImageUploader onSave={(url: any) => setCollection({ ...collection, image: url })} />
                )
              }
            </Card>
...

      {
        collection.collectionType === "auto" && (
          <div className="flex flex-col gap-4 mt-2 items-start">
            <FilledButton disabled={loading} onClick={() => {
