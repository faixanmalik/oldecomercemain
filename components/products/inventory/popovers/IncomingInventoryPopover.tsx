...
export default function IncomingInventoryPopover({ quantity }: { quantity: number }) {
      ...
      <PopoverContent className="rounded-xl items-start w-60 whitespace-nowrap p-2 bg-white flex gap-1 flex-col">
        {
          quantity > 0 ? (
            ...
            <Text className="text-gray-800" as={'div'}>
              Create <Link href="/products/transfers/new"><TextButton>transfer</TextButton></Link> or <Link href="/products/purchase_orders/new"><TextButton>purchase order</TextButton></Link>
            </Text>
          )
        }
