...
export default function RemoveProductsFromCollectionsDialog({
  selectedProducts,
}: {
  ...
  async function fetchCollections() {
    try {
      const { data } = await axios.get(
        "/api/products/collections?fields=_id,title"
      );
      setCollections(data);
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="h-min">
        <DialogHeader>
          <DialogTitle>
            Remove {selectedProducts.length} product(s) to collecitons
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="p-4 gap-4 flex flex-col">
          <Input
            ...
            icon={<FaSearch size={14} className="text-gray-500" />}
          />
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    ...
                    setSelectedCollections(
                      selectedCollections.filter((tag) => tag !== c)
                    );
                  }
                }}
              />
