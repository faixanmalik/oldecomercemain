...
export default function RemoveTagsFromProductsDialog({ selectedProducts }: { selectedProducts: Product[] }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="h-min">
        <DialogHeader>
          <DialogTitle>Remove tags from {selectedProducts.length} product(s)</DialogTitle>
        </DialogHeader>

        <div className="flex flex-col p-4">
          <Input id="search-tags" placeholder="Search to find tags" onChange={e => handleSearch(e.target.value)} icon={<FaSearch size={14} className="text-gray-800" />} />
